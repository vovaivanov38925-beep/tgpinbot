...
if RICH:
    console.print(Panel.fit(
        "[bold cyan]Pinterest Scraper v30[/bold cyan]\n[dim]564x images + more scrolls[/dim]",
        title="🚀 Scraper"
    ))
...
def scrape(url):
    ...
    try:
        ...
        document.querySelectorAll('a[href*="/pin/"]').forEach(a => {
            ...
            // Конвертируем в 564x (не originals - он заблокирован)
            imageUrl = imageUrl.replace(/\\/\\d+x\\d*\\//g, '/564x/');
            imageUrl = imageUrl.replace('/236x/', '/564x/');
            imageUrl = imageUrl.replace('/474x/', '/564x/');
